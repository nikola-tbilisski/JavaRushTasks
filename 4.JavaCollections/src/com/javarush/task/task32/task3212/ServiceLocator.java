...
public class ServiceLocator {
    ...
    public static Service getService(String jndiName) {
        Service tempGetService = cache.getService(jndiName);

        if(tempGetService != null){
            return tempGetService;
        }else {
            InitialContext in = new InitialContext();
            Object ob = in.lookup(jndiName);

            if(ob != null){
                tempGetService = (Service) ob;
                cache.addService(tempGetService);
                return tempGetService;
            }
        }
