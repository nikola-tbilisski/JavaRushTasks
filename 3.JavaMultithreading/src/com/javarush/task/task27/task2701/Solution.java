...
public class Solution {
    ...
    static boolean isSubstringPresent(String substring, String string) {
        int substringCount = 0, stringCount = 0;

        while (substringCount < substring.length() && stringCount < string.length()) {
            if(substring.charAt(substringCount) == string.charAt(stringCount)) substringCount ++;
            stringCount ++;
        }
