package com.javarush.task.task28.task2801;

/* 
Осваиваем switch
*/

public class Solution {
    public static enum E1 {A, B, C, Y}

    public static enum E2 {D, E, F}

    public static enum E3 {D, E, F}

    public static void main(String[] args) {
        Solution.switchTest(E1.C);
        Solution.switchTest(E3.D);
        Solution.switchTest(E2.D);
        Solution.switchTest(E2.F);
        /* output
        it's E1.C
        undefined
        it's E2.D
         */
    }

    public static void switchTest(Enum obj) {
        switch (obj.getClass().getSimpleName()){
            case "E1":
                //E1[] e1Values = E1.values();
//                for(E1 e1 : E1.values()){
//                    if(e1.equals(obj)) System.out.println("it's " + obj.getClass().getSimpleName() + "." + obj);
//                }
                System.out.println("it's " + obj.getClass().getSimpleName() + "." + obj);
                break;

            case "E2":
//                for(E2 e2 : E2.values()){
//                    if(e2.equals(obj)) System.out.println("it's " + obj.getClass().getSimpleName() + "." + obj);
//                }
                System.out.println("it's " + obj.getClass().getSimpleName() + "." + obj);
                break;

            default:
                System.out.println("undefined");
        }
        //add your code here
    }
}
